...
int udp_recv(void)
{
	...
	mreq.imr_multiaddr.s_addr = inet_addr("224.0.0.77");// 多播地址，类似于 QQ 群号
	...
	while(1)
	{
		// 接收数据 
		memset(buff, 0, 50); 
		n = recvfrom(sockfd, buff, sizeof(buff),0,(struct sockaddr*)&local_addr, (socklen_t *)&addr_len);    
		if( n > 0 )
		{
			printf("recv ip:%s\n",buff);
